...
import { companyProfilePath, companyProfiles } from "@/lib/company-profiles";
...
export default function sitemap(): MetadataRoute.Sitemap {
 ...
 const companyPages=companyProfiles.map(profile=>({url:site.domain+companyProfilePath(profile.companyId),lastModified:new Date(profile.reviewedOn),changeFrequency:"monthly" as const,priority:0.7}));
 return [...staticPages,...cataloguePages,...sectorPages,...companyPages,...blogPages,...blogCategoryPages];
}
