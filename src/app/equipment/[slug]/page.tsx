...
import { catalogue, getGroup, requestUrl, sectors, typeUrl } from "@/lib/catalogue";
...
export default async function CategoryPage({params}:{params:Promise<{slug:string}>}){
 ...
 return <>
  ...
  <section className="mx-auto grid max-w-6xl gap-8 px-4 py-12 lg:grid-cols-[minmax(0,1fr)_280px]">
   <div className="min-w-0"><h2 className="text-2xl font-bold">Requirements & enquiry checklists</h2><p className="mt-3 text-muted">Select the closest requirement below. The checklist helps us request a comparable quotation; attach your datasheet or drawing when available.</p>
    ...
    <div className="space-y-6">{group.types.map(t=><article id={t.id} key={t.id} className="scroll-mt-24 rounded-xl border border-line bg-white p-6"><p className="mb-2 text-xs font-semibold uppercase tracking-wide text-accent">{t.kind ?? group.kind}</p><h3 className="text-xl font-semibold">{t.detail ? <Link href={typeUrl(group,t)} className="hover:text-accent">{t.name}</Link> : t.name}</h3><p className="mt-3 leading-relaxed text-muted">{t.description}</p>{t.detail && <Link href={typeUrl(group,t)} className="mt-4 inline-block text-sm font-semibold text-accent">Read the detailed procurement brief →</Link>}<h4 className="mt-5 font-semibold">Include in your request</h4><ul className="mt-3 grid list-inside list-disc gap-2 text-muted sm:grid-cols-2">{t.requirements.map(r=><li key={r}>{r}</li>)}</ul><Link className="mt-6 inline-block rounded border border-line px-4 py-3 font-semibold text-accent hover:border-accent" href={requestUrl(slug,t.id)}>Request {t.name.toLowerCase()} →</Link></article>)}</div>
