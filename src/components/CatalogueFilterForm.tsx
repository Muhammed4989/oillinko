...
export default function CatalogueFilterForm({ filters }: { filters: CatalogueFilters }) {
  ...
  useEffect(() => {
    if (window.location.search || !window.location.hash.includes("=")) return;
  ...
  return <form action="/equipment" method="get" onSubmit={event => {
    ...
    <label className="mt-5 block text-sm font-medium" htmlFor="catalogue-category">Equipment / service category</label><select id="catalogue-category" name="category" defaultValue={filters.category} className={control}><option value="">All categories</option>{catalogue.map(g => <option key={g.slug} value={g.slug}>{g.name}</option>)}</select>
    ...
    <label className="mt-5 block text-sm font-medium" htmlFor="catalogue-kind">Requirement type</label><select id="catalogue-kind" name="kind" defaultValue={filters.kind} className={control}><option value="">All requirement types</option><option>Equipment</option><option>Service</option><option>Software</option></select>
    ...
    <Link href="/equipment" className="mt-4 inline-block text-sm font-semibold text-accent underline">Clear selections</Link>
  </form>;
}
